...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth,page=2,trim=0 2.5cm 0 0.5cm,clip]{pinn_schematics.pdf}
\caption{Complete Data Flow Pipeline: From data generation through training to evaluation. The pipeline includes trajectory generation (10 sequences, 49,382 samples), an 80/20 train-test split, curriculum learning over 250 epochs, and holdout evaluation achieving 51$	imes$ improvement over baseline (0.029m vs 1.49m at 100-step horizon).}
\label{fig:data_flow}
\end{figure}
...
12 & Enhanced Physics & 10$	imes$ physics loss increase & 8.9\% error \\
...
Dropout 1 & 256 & 256 & 0 & Regularization (p=0.3) \\
Layer 2 & 256 & 256 & 65,792 & Nonlinear dynamics \\
Dropout 2 & 256 & 256 & 0 & Regularization (p=0.3) \\
Layer 3 & 256 & 256 & 65,792 & Deep features \\
Dropout 3 & 256 & 256 & 0 & Regularization (p=0.3) \\
Layer 4 & 256 & 256 & 65,792 & Higher-order dynamics \\
Dropout 4 & 256 & 256 & 0 & Regularization (p=0.3) \\
...
\textbf{Note:} Deep feed-forward architecture with 5 hidden layers of 256 neurons each. Takes 16D input (12 states + 4 controls) and outputs 12D next states. Dropout regularization (p=0.3) between layers improves autoregressive stability and prevents overfitting. The network predicts all 12 next state variables (x, y, z, $\phi$, $\theta$, $\psi$, p, q, r, $v_x$, $v_y$, $v_z$), with 6 learned physical parameters (m, Jxx, Jyy, Jzz, kt, kq). Gravity (g=9.81 m/s²) is a \textbf{fixed constant} used in physics calculations, NOT a learnable parameter.


\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth,page=1]{pinn_schematics.pdf}
...
\subsubsection{Inputs to PINN Model (16 Variables)}
\begin{table*}[!t]
\centering
\renewcommand{\arraystretch}{1.4}
...
\midrule
\multicolumn{6}{c}{\textit{State Variables (12)}} \\
1 & \textbf{x} & $x$ & m & X position (inertial frame) & [-50.0, 50.0] \\
2 & \textbf{y} & $y$ & m & Y position (inertial frame) & [-50.0, 50.0] \\
3 & \textbf{z} & $z$ & m & Vertical position (NED: $z<0$ = above ground) & [-25.0, 0.0] \\
4 & \textbf{roll} & $\phi$ & rad & Roll angle (banking) & [$-\pi/4$, $\pi/4$] \\
5 & \textbf{pitch} & $\theta$ & rad & Pitch angle (nose up/down) & [$-\pi/4$, $\pi/4$] \\
6 & \textbf{yaw} & $\psi$ & rad & Yaw angle (heading) & [$-\pi$, $\pi$] \\
7 & \textbf{p} & $p$ & rad/s & Roll rate (angular velocity) & [-10.0, 10.0] \\
8 & \textbf{q} & $q$ & rad/s & Pitch rate (angular velocity) & [-10.0, 10.0] \\
9 & \textbf{r} & $r$ & rad/s & Yaw rate (angular velocity) & [-5.0, 5.0] \\
10 & \textbf{vx} & $v_x$ & m/s & X velocity (body frame) & [-10.0, 10.0] \\
11 & \textbf{vy} & $v_y$ & m/s & Y velocity (body frame) & [-10.0, 10.0] \\
12 & \textbf{vz} & $v_z$ & m/s & Z velocity (vertical) & [-10.0, 10.0] \\
\midrule
\multicolumn{6}{c}{\textit{Control Inputs (4)}} \\
13 & \textbf{thrust} & $T$ & N & Total thrust force from 4 motors & [0.0, 2.0] \\
14 & \textbf{torque\_x} & $\tau_x$ & N$\cdot$m & Roll torque (about x-axis) & [-0.02, 0.02] \\
15 & \textbf{torque\_y} & $\tau_y$ & N$\cdot$m & Pitch torque (about y-axis) & [-0.02, 0.02] \\
16 & \textbf{torque\_z} & $\tau_z$ & N$\cdot$m & Yaw torque (about z-axis) & [-0.01, 0.01] \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{center}
\texttt{INPUT VECTOR (16$	imes$1) $\rightarrow$ NEURAL NETWORK $\rightarrow$ OUTPUT VECTOR (12+6)}



$[x, y, z, \phi, \theta, \psi, p, q, r, v_x, v_y, v_z, T, \tau_x, \tau_y, \tau_z]_t$

$\downarrow$

\textbf{PHYSICS-INFORMED NEURAL NETWORK}
(5 layers $\times$ 256 neurons + 6 learnable parameters)

$\downarrow$

$[x, y, z, \phi, \theta, \psi, p, q, r, v_x, v_y, v_z]_{t+1}$ + Learned: $[m, J_{xx}, J_{yy}, J_{zz}, k_t, k_q]$
\end{center}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth,page=4,trim=0 2.5cm 0 0.5cm,clip]{pinn_schematics.pdf}
\caption{Multi-Objective Loss Function: The total loss combines multiple terms enforcing specific constraints. Weighted components include: Data loss ($\lambda=1.0$), Physics loss ($\lambda=20$), Energy conservation, Temporal smoothness ($\lambda=2.0$), Stability bounds ($\lambda=0.05$), and L2 Regularization. Specific weights may vary across training configurations.}
...
\begin{tabular}{p{0.18\textwidth}p{0.57\textwidth}p{0.13\textwidth}}
\toprule
...
\noindent Where: $t_1 = (J_{yy} - J_{zz})/J_{xx}$, $t_2 = (J_{zz} - J_{xx})/J_{yy}$, $t_3 = (J_{xx} - J_{yy})/J_{zz}$
...
\noindent Where: $f_x = 0$, $f_y = 0$, $f_z = -T$ (thrust acts along body z-axis), $c_d = 0.05$ kg/m (quadratic drag coefficient)
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth,page=2,trim=0 4.5cm 0 1.5cm,clip]{pinn_schematics.pdf}
...
\small
\renewcommand{\arraystretch}{1.5}
\begin{tabular}{>{\raggedright\arraybackslash}p{3.2cm}>{\raggedright\arraybackslash}p{6.5cm}>{\raggedright\arraybackslash}p{4.5cm}}
\toprule
...
\textbf{Enhanced Architecture} & 256 neurons, 5 layers, dropout (p=0.3) & Improved capacity and stability \\
\textbf{Scheduled Sampling} & Gradual autoregressive training (0$\rightarrow$30\%) & Robust multi-step predictions \\
...
\bottomrule
\end{tabular}
\caption{Key features of the PINN implementation showing comprehensive integration of physics-informed constraints and architectural innovations.}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth,page=4,trim=0 0.2cm 0 1cm,clip]{pinn_schematics.pdf}
\caption{PINN v2 Performance on Held-Out Test Set: The optimized model achieves 51$	imes$ improvement over baseline at 100-step horizon (0.029m vs 1.49m), with state-wise improvements ranging from 2$	imes$ to 51×. The model demonstrates exceptional stability with only 1.1$	imes$ error growth (vs 17$	imes$ for baseline) and accurate physics parameter identification (mass and motor coefficients at 0\% error, inertias within 5\%).}
...
\textbf{FINAL RESULTS WITH TEMPORAL SMOOTHNESS:} Real physics + temporal constraints. Physics weight: 20.0, temporal weight: 2.0, stability weight: 0.05.
...
\textbf{Jxx} & 6.86e-5 & 7.21e-5 & 3.5e-6 & \textcolor{blue}{\textbf{5.0\%}} & \textcolor{green}{Excellent} \\
\textbf{Jyy} & 9.20e-5 & 9.66e-5 & 4.6e-6 & \textcolor{blue}{\textbf{5.0\%}} & \textcolor{green}{Excellent} \\
\textbf{Jzz} & 1.37e-4 & 1.43e-4 & 6.8e-6 & \textcolor{blue}{\textbf{5.0\%}} & \textcolor{green}{Excellent} \\
\bottomrule
\end{tabular}
\normalsize
\end{table*}
...
\textbf{Summary:} The PINN achieves \textbf{perfect identification} for kt, kq, and mass (0\% error). Inertia parameters show 5\% error, which is excellent for quadrotor applications and demonstrates successful learning without artificial physics terms.
...
\end{itemize}
\item State vector: 16 features (12 states + 4 controls)
\end{itemize}
...
\textbf{Training Config} & Batch size 64, 250 epochs (with curriculum learning), physics weight 20.0, regularization weight 1.0, gravity g=9.81 m/s$^2$ fixed \\
...
\hline
Baseline & Jxx/Jyy/Jzz & 1300-6700\% \\
After Option 1 & Jxx/Jyy/Jzz & 30-46\% \\
\textbf{Final} & \textbf{ALL 6} & \textbf{0.00\%, 5.00\%} \\
\hline
\end{tabular}
\caption{Parameter identification improvement progression}
\end{table}
...
\textit{Note: Each figure below shows all 10 training trajectories as separate subplots (5$	imes$2 grid), allowing individual trajectory analysis rather than overlaid visualization. This eliminates visual clutter and makes it easier to examine the model's performance on each trajectory independently.}

\vspace{0.3cm}
\noindent\fcolorbox{black}{yellow!15}{%
\parbox{0.97\columnwidth}{%
\textbf{IMPORTANT -- One-Step vs. Autoregressive Prediction:}\\[0.1cm]
The plots below show \textbf{one-step prediction} performance, where at each timestep $t$, the model receives the \textit{ground truth state} as input and predicts the state at $t+1$ (i.e., $\Delta t = 1$ms ahead). This evaluates whether the PINN correctly learned the underlying \textbf{Newton-Euler dynamics}---a model with incorrect dynamics would produce poor predictions regardless of input quality.\\[0.1cm]
This differs from \textbf{autoregressive rollout}, where predictions feed back as inputs and errors accumulate over time. One-step evaluation isolates the model's ability to capture instantaneous dynamics without compounding prediction errors. For multi-step horizon analysis, see Fig.~\ref{fig:autoregressive_stability}.
}}
\vspace{0.3cm}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/01_x_diverse_test.png}
\caption{X Position vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.118 m}, RMSE: \textbf{0.180 m}). Each subplot shows one test trajectory with its true values (blue) and PINN predictions (red dashed). Horizontal position prediction in inertial frame from diverse training set.}
\label{fig:x_position_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/02_y_diverse_test.png}
\caption{Y Position vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.100 m}, RMSE: \textbf{0.166 m}). Each subplot shows one test trajectory with its true values (blue) and PINN predictions (red dashed). Horizontal position prediction in inertial frame from diverse training set.}
\label{fig:y_position_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/03_z_diverse_test.png}
\caption{Z Position (Altitude) vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.243 m}, RMSE: \textbf{0.363 m}). Each subplot shows one test trajectory with its true values (blue) and PINN predictions (red dashed). Vertical position tracking by PINN from diverse training set.}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/04_roll_diverse_test.png}
\caption{Roll Angle ($\phi$) vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.0030 rad (0.17°)}, RMSE: \textbf{0.0045 rad}). Each subplot shows one trajectory with its true values (blue) and PINN predictions (red dashed). Roll angle prediction from diverse training set.}
\label{fig:roll_angle_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/05_pitch_diverse_test.png}
\caption{Pitch Angle ($\theta$) vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.0056 rad (0.32°)}, RMSE: \textbf{0.0075 rad}). Each subplot shows one trajectory with its true values (blue) and PINN predictions (red dashed). Pitch angle prediction from diverse training set.}
\label{fig:pitch_angle_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/06_yaw_diverse_test.png}
\caption{Yaw Angle ($\psi$) vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.0069 rad (0.40°)}, RMSE: \textbf{0.0095 rad}). Each subplot shows one trajectory with its true values (blue) and PINN predictions (red dashed). Yaw angle prediction from diverse training set.}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/07_p_diverse_test.png}
\caption{Roll Rate (p) vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.0032 rad/s}, RMSE: \textbf{0.0045 rad/s}). Each subplot shows one trajectory with its true values (blue) and PINN predictions (red dashed). Roll rate prediction from diverse training set.}
\label{fig:roll_rate_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/08_q_diverse_test.png}
\caption{Pitch Rate (q) vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.0029 rad/s}, RMSE: \textbf{0.0063 rad/s}). Each subplot shows one trajectory with its true values (blue) and PINN predictions (red dashed). Pitch rate prediction from diverse training set.}
\label{fig:pitch_rate_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/09_r_diverse_test.png}
\caption{Yaw Rate (r) vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.0028 rad/s}, RMSE: \textbf{0.0067 rad/s}). Each subplot shows one trajectory with its true values (blue) and PINN predictions (red dashed). Yaw rate prediction from diverse training set.}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/10_vx_diverse_test.png}
\caption{X Velocity (vx, inertial frame) vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.186 m/s}, RMSE: \textbf{0.217 m/s}). Each subplot shows one trajectory with its true values (blue) and PINN predictions (red dashed). Horizontal velocity component from diverse training set.}
\label{fig:vx_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/11_vy_diverse_test.png}
\caption{Y Velocity (vy, inertial frame) vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.258 m/s}, RMSE: \textbf{0.334 m/s}). Each subplot shows one trajectory with its true values (blue) and PINN predictions (red dashed). Horizontal velocity component from diverse training set.}
\label{fig:vy_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/12_vz_diverse_test.png}
\caption{Z Velocity (vz, vertical) vs Time - Test Set (15 Held-Out Trajectories) (Overall MAE: \textbf{0.253 m/s}, RMSE: \textbf{0.364 m/s}). Each subplot shows one trajectory with its true values (blue) and PINN predictions (red dashed). Vertical velocity tracking from diverse training set.}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/13_thrust_diverse_test.png}
\caption{Total Thrust vs Time - Test Set (15 Held-Out Trajectories). \textbf{Control input (NOT predicted)} showing realistic motor dynamics with 80ms time constant. Each subplot shows one trajectory's thrust command profile from diverse test set.}
\label{fig:thrust_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/14_torque_x_diverse_test.png}
\caption{Roll Torque ($\tau_x$) vs Time - Test Set (15 Held-Out Trajectories). \textbf{Control input (NOT predicted)} for roll attitude maneuvers. Each subplot shows one trajectory's roll torque command profile from diverse test set.}
\label{fig:torque_x_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/15_torque_y_diverse_test.png}
\caption{Pitch Torque ($\tau_y$) vs Time - Test Set (15 Held-Out Trajectories). \textbf{Control input (NOT predicted)} for pitch attitude maneuvers. Each subplot shows one trajectory's pitch torque command profile from diverse test set.}
\label{fig:torque_y_analysis}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{../results/test_set_trajectories_diverse/16_torque_z_diverse_test.png}
\caption{Yaw Torque ($\tau_z$) vs Time - Test Set (15 Held-Out Trajectories). \textbf{Control input (NOT predicted)} for yaw (heading) maneuvers. Each subplot shows one trajectory's yaw torque command profile from diverse test set.}
...
\begin{itemize}
\item Loss weight: $\lambda_{temporal} = 2.0$ (balanced with $\lambda_{physics} = 20.0$)
...
\begin{equation*}
\dot{p} \approx \frac{\tau_x}{J_{xx}} - 2p
\end{equation*}
\textit{Note: Cross-coupling term $\frac{J_{yy} - J_{zz}}{J_{xx}} q \cdot r \approx 0$ when $q, r$ are small.}
...
\begin{align*}
\mathcal{L}_{\text{ang\_accel}} = \frac{1}{N}\sum_{i=1}^{N} &\left[(\dot{p}_{\text{pred}} - \dot{p}_{\text{physics}})^2 \right. \\
&\left. + (\dot{q}_{\text{pred}} - \dot{q}_{\text{physics}})^2 + (\dot{r}_{\text{pred}} - \dot{r}_{\text{physics}})^2\right]
\end{align*}
where $\dot{p}_{\text{physics}} = \frac{J_{yy} - J_{zz}}{J_{xx}} q r + \frac{\tau_x}{J_{xx}} - 2p$
...
Despite achieving lower parameter count (65\% reduction) and improved single-step performance (2-10$	imes$ better accuracy), optimized model variants exhibited \textbf{severe instability during long-horizon autoregressive rollouts}, with errors 100-3,500,000$	imes$ worse than baseline. This indicates a fundamental trade-off between architectural complexity and autoregressive stability for control applications.
...
\begin{itemize}
\item \textbf{Single-step teacher-forced MAE:} Excellent (< 0.01 m for z, 2-10$	imes$ better than baseline)
\item \textbf{100-step autoregressive MAE:} Catastrophic (177 m to 5.2M m for z, 100-3,500,000$	imes$ worse than baseline)
...
\textbf{Key Finding:} Baseline PINN wins on \textbf{ALL autoregressive metrics} (the critical metrics for control) despite having 3$	imes$ more parameters and lower single-step accuracy.
...
\midrule
Baseline architecture & 1.0$	imes$ & 250 epochs \\
+ Residual connections & 1.2$	imes$ & 300 epochs \\
+ Modular design & 1.5$	imes$ & 375 epochs \\
+ Multi-step rollout & 2.0$	imes$ & 500 epochs \\
...
\item \textbf{Gap:} 20$	imes$ mismatch
\end{itemize}
...
\textbf{Question:} How can a model be 10$	imes$ better at single-step but 100$	imes$ worse at multi-step?
...
\item Simpler architecture $\rightarrow$ fewer failure modes
\end{itemize}
...
\item \textbf{Residual Connections + Swish Activation:}
  \begin{itemize}
  \item Better gradient flow, 2$	imes$ faster convergence
...
\item \textbf{Multi-Step Rollout Loss:}
  \begin{itemize}
  \item Reduced rollout error 6.8$	imes$ (197,963 $\rightarrow$ 29,004)
...
\item \textbf{Stability matters more than precision:} For control tasks, robustness to compounding error outweighs marginal accuracy gains. A model that's 10$	imes$ more accurate per step but diverges after 60 steps is useless.
...
\textbf{Result: 51$	imes$ improvement over baseline (0.029m vs 1.49m at 100-step horizon) with 91.4\% average improvement across all 12 dynamical states.}
...
\caption{Ablation Study: Progressive Impact of Optimization Components on 100-Step MAE. Each component addition yields substantial improvements: Baseline (1.49m) $\rightarrow$ +Curriculum Learning (0.82m, 45\% reduction) $\rightarrow$ +Scheduled Sampling (0.45m, 45\% further reduction) $\rightarrow$ +Dropout Regularization (0.12m, 73\% reduction) $\rightarrow$ +Energy Conservation Loss (0.029m, 76\% reduction). The cumulative 51$	imes$ improvement demonstrates the synergistic effect of combining stability mechanisms systematically rather than in isolation.}
...
\item \textbf{Regularization:} Dropout 0.3, AdamW weight decay $10^{-4}$
...
\textbf{Critical observation:} Minimal error growth from 1 to 100 steps (0.026m $\rightarrow$ 0.029m, 1.1$	imes$ growth), proving \textit{exceptional dynamic stability on completely unseen data}. Note that error actually \textit{decreased} from 1 to 10 steps (0.026m $\rightarrow$ 0.017m), demonstrating true learned dynamics rather than memorization.
...
\midrule
z (m) & 1.490 & \textbf{0.029} & +98.0\% & 51$	imes$ better \\
vz (m/s) & 1.550 & \textbf{0.038} & +97.6\% & 41$	imes$ better \\
roll (rad) & 0.018 & \textbf{0.0011} & +93.6\% & 16$	imes$ better \\
pitch (rad) & 0.003 & \textbf{0.0003} & +89.2\% & 11$	imes$ better \\
yaw (rad) & 0.032 & \textbf{0.0028} & +91.3\% & 11$	imes$ better \\
p (rad/s) & 0.067 & \textbf{0.0354} & +47.2\% & 1.9$	imes$ better \\
q (rad/s) & 0.167 & \textbf{0.0253} & +84.9\% & 6.6$	imes$ better \\
r (rad/s) & 0.084 & \textbf{0.0278} & +66.9\% & 3.0$	imes$ better \\
\multicolumn{3}{l}{\textbf{Average improvement:}} & \textbf{83.6\%} & \textbf{18$	imes$ better} \\
\bottomrule
\end{tabular}
...
10 $\rightarrow$ 50 steps & 0.162m $\rightarrow$ 0.521m (3.2×) & 0.017m $\rightarrow$ 0.021m (1.24×) & \textbf{2.6$	imes$ slower} \\
50 $\rightarrow$ 100 steps & 0.521m $\rightarrow$ 1.490m (2.9×) & 0.021m $\rightarrow$ 0.029m (1.39×) & \textbf{2.1$	imes$ slower} \\
\textbf{Overall} & \textbf{17$	imes$ growth} & \textbf{1.1$	imes$ growth} & \textbf{15$	imes$ more stable} \\
\bottomrule
\end{tabular}
\caption{Error growth comparison on held-out test set proving exceptional long-term stability}
\end{table*}

The error showed \textbf{minimal growth} (1.1$	imes$ total) from 1 to 100 steps on held-out data, and actually \textit{decreased} from 1 to 10 steps, demonstrating that the model learned true dynamics rather than memorizing training data. This proves exceptional dynamic stability that baseline completely lacks.
...
\textbf{Optimized v2} & \textbf{All baseline components maintained} & \textbf{0.029 m - 51$	imes$ improvement (holdout)} \\
\bottomrule
\end{tabular}
...
\item \textbf{Complete optimization solution:} Developed and validated 10-step methodology achieving 49$	imes$ improvement while maintaining dynamic stability
...
\item Quadratic drag terms $(v^2)$ introduce 3--5$	imes$ higher damping than linear model predicts
...
\begin{itemize}
\item \textbf{Empirical demonstration of PINN vulnerability}: Quantifies the impact of physics model mismatch on parameter estimation (5\% $\rightarrow$ 46\% error despite 42\% better state prediction).
...
Through systematic implementation of all 10 stability techniques, we achieved \textbf{91.4\% average improvement} (49$	imes$ better for position tracking) over the baseline PINN. This demonstrates that architectural optimizations CAN dramatically improve autoregressive prediction performance when:
