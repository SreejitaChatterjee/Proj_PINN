...
\textbf{Important Note on Simulation Limitations:} The training data generation does not model actuator dynamics (motor time constants, bandwidth limitations, or first-order lag). Control outputs (thrust, torques) are applied instantaneously to the system dynamics, resulting in sharp, step-like transitions in control plots. Real quadrotor systems exhibit smoother control profiles due to motor electrical/mechanical time constants (typically 50-100ms). This simplification focuses the PINN on learning core rigid-body dynamics rather than actuator-level details. Future work should incorporate motor dynamics for higher-fidelity simulation.
...
\hline
\textbf{Training Config} & Batch size 64 (foundation) / 128 (improved/advanced), 50-150 epochs, physics weight 0.1-2.0 \\
\hline
...
\textbf{Training Data Uses Square Wave Inputs (Not Constant):} All 10 trajectories use square wave reference inputs for roll, pitch, yaw, and altitude. This provides rich dynamic behavior with periodic step changes, testing the controller's transient response repeatedly throughout each 5-second flight.
...
\textbf{Important Note on Convergence Visualization:} The parameter convergence plots (Figures 13-16) represent \textit{idealized simulation curves} demonstrating expected learning behavior, NOT actual recorded training data. These visualizations illustrate typical exponential convergence patterns toward true parameter values. Actual training would show more noise and oscillation. The final parameter values and errors reported in tables are from actual trained models.
...
\caption{Mass Parameter Learning Convergence (Idealized Simulation) - Illustrative convergence curve showing typical learning behavior for vehicle mass identification with 4.4\% final error achieved at epoch 48 in actual training.}
...
\caption{X-axis Moment of Inertia Learning (Idealized Simulation) - Illustrative convergence showing typical learning pattern with 5.4\% final error, stable identification achieved at epoch 62 in actual training.}
...
\caption{Y-axis Moment of Inertia Learning (Idealized Simulation) - Illustrative convergence demonstrating expected learning behavior with 7.3\% final error, convergence at epoch 58 in actual training.}
...
\caption{Z-axis Moment of Inertia Learning (Idealized Simulation) - Illustrative convergence analysis showing typical learning pattern with 5.6\% identification error, stable at epoch 55 in actual training.}
...
\caption{Physical Parameter Identification Analysis (Idealized Simulations) - Illustrative learning convergence curves for all 6 physical parameters (mass, Jxx, Jyy, Jzz, kt, kq) demonstrating typical training behavior over 120 epochs. Each subplot shows: blue curve = idealized PINN learning trajectory starting from initial guess, red dashed line = true parameter value, blue dotted line = final learned value from actual training, green dot = initial value. Final errors from actual trained models: Mass = 4.4\%, Jxx = 5.4\%, Jyy = 7.3\%, Jzz = 5.6\%, kt = 2.0\%, kq = 1.8\%. Note: These curves represent idealized convergence patterns; actual training exhibits more variation.}
...
\begin{itemize}
\item \textbf{Actuator Dynamics Not Modeled}: Training data does not include motor time constants, bandwidth limitations, or first-order lag effects. Control outputs (thrust, torques) are applied instantaneously, resulting in sharp transitions measured at 481.64 N/s thrust slew rate (real systems: 50-100 N/s).
\item \textbf{Overly Aggressive Controller}: Vertical velocities reach -5.24 m/s and accelerations reach 1.01g, indicating controller gains that would be unsafe on real hardware.
...
\item \textbf{Idealized Convergence Visualization}: Parameter convergence plots (Figures 13-16) are idealized simulations showing expected patterns, not actual noisy training curves. Final values are from real training.
...
\textbf{Future Work Directions (Prioritized by Impact):}
\begin{enumerate}
\item \textbf{Add Motor Dynamics Model}: Implement first-order lag $\dot{T} = (T_{cmd} - T)/\tau_m$ with $\tau_m = 50$ms and slew rate limits of 50-100 N/s. This will eliminate unrealistic 481.64 N/s thrust changes.
\item \textbf{Reduce Controller Aggressiveness}: Tune PID gains to limit vertical velocities to $<$3 m/s and accelerations to $<$0.5g. Current 1.01g accelerations are unsafe for real deployment.
\item \textbf{Add Control Input Filtering}: Low-pass filter PID outputs with cutoff frequency $\sim$20 Hz to smooth torque commands and prevent sharp discontinuities visible in Figures 3-5.
\item \textbf{Implement Actuator Rate Limits}: Add $d\tau/dt$ constraints ($<$0.5 N$\cdot$m/s) to prevent instantaneous torque steps that violate real actuator physics.
\item \textbf{Enhance Physics Loss}: Add penalty terms for:
   \begin{itemize}
   \item Control input derivatives: $\lambda_{ctrl} \|\dot{T}\|^2 + \|\dot{\tau}\|^2$
   \item State derivative smoothness: $\lambda_{smooth} \|\ddot{x}\|^2$
   \item Physical acceleration limits: penalty for $|\ddot{z}| > 0.5g$
   \end{itemize}
\item \textbf{Validate Against Real Hardware}: Test learned dynamics on Crazyflie 2.0 with proper motor dynamics to quantify sim-to-real gap caused by current unrealistic transients.
\item \textbf{Add Measurement Noise}: Include sensor noise and IMU dynamics for robustness testing.
\item \textbf{Extend Operating Envelope}: Gradually increase to aggressive maneuvers after fixing baseline realism issues.
\end{enumerate}

\textbf{Expected Improvements from Priority 1-4:}
\begin{itemize}
\item Thrust slew rate: 481.64 N/s $\rightarrow$ $<$100 N/s (4.8× reduction)
\item Vertical velocity: -5.24 m/s $\rightarrow$ $<$3 m/s (1.7× reduction)
\item Torque smoothness: Eliminate discontinuities, continuous differentiable profiles
\item Better parameter identification: Inertia estimation less biased by unrealistic accelerations
\item Improved sim-to-real transfer: Training data more representative of real quadrotor behavior
\end{itemize}
