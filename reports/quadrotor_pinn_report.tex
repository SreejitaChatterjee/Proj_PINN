...
\textbf{Important Distinction - Reference vs. Response:} The square wave signals are \textit{reference setpoints} (desired values) for the PID controller, NOT the actual system trajectories. The actual attitude angles and altitude follow smooth, continuous paths as the quadrotor physically tracks these step-change references. Plots show both: (1) red dashed lines indicating square wave setpoints, and (2) blue curves showing the smooth system response. The smooth response is physically correct - quadrotors cannot instantaneously change angles or altitude. This tracking behavior validates realistic controller performance with rise times, overshoot, and settling characteristics.
...
\caption{Thrust Force vs Time (Trajectory 0) - Three-phase thrust profile demonstrating physically realistic quadrotor dynamics: climb phase at 1.334N (t = 0-2s), transition phase with controlled deceleration (t = 2-4s), and steady hover at 0.667N = m×g (t = 4-5s). Red dashed reference line shows hover thrust equilibrium. This behavior validates Newton's second law implementation in the PINN model.}
...
\caption{Vertical Position (Altitude) vs Time (Trajectory 0) - Square wave tracking between 3.0m and 5.0m altitude setpoints (period: 2.0s, red dashed lines). The plot shows height (h=-z) to match conventional altitude representation. PID altitude controller (kz1=2.0, kz2=0.22, kv=-0.4 for NED coordinates) demonstrates effective tracking of time-varying reference commands with smooth transitions between setpoint levels.}
...
\caption{Roll Angle vs Time (Trajectory 0) - Square wave tracking between -10.0° and +10.0° setpoints (period: 2.0s, red dashed lines) using cascaded PID control (outer loop k1=1.0, ki=0.004; inner loop k2=0.1). Attitude displayed in degrees after conversion from radians ($\phi_{deg} = \phi_{rad} \times 180/\pi$). Smooth tracking with minimal oscillation demonstrates effective roll control and proper Euler equation implementation: $\dot{p} = (J_{yy}-J_{zz})qr/J_{xx} + \tau_x/J_{xx} - c_\omega p$ where $c_\omega = 2.0$ s$^{-1}$.}
...
\caption{Pitch Angle vs Time (Trajectory 0) - Square wave tracking between -5.0° and +5.0° setpoints (period: 2.5s, red dashed lines). Cascaded PID controller (k11=1.0, ki1=0.004, k21=0.1) maintains stable pitch attitude through transitions. Cross-coupling with roll and yaw visible through Euler equation: $\dot{q} = (J_{zz}-J_{xx})pr/J_{yy} + \tau_y/J_{yy} - c_\omega q$ where $c_\omega = 2.0$ s$^{-1}$. Smooth response validates PINN's ability to learn coupled rotational dynamics.}
...
\caption{Yaw Angle vs Time (Trajectory 0) - Square wave tracking between -5.0° and +5.0° setpoints (period: 3.0s, red dashed lines) demonstrating directional maneuvers. PID controller (k12=1.0, ki2=0.004, k22=0.1) regulates yaw with smooth transitions. Yaw dynamics governed by: $\dot{r} = (J_{xx}-J_{yy})pq/J_{zz} + \tau_z/J_{zz} - c_\omega r$ where $c_\omega = 2.0$ s$^{-1}$. Independent yaw control during simultaneous roll and pitch maneuvers validates proper implementation of decoupled heading control.}
