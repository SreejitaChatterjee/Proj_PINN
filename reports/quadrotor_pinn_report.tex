...
\item \textbf{Inertia Parameter Errors}: While mass/kt/kq achieve 0\% error, inertias show 30-46\% errors (Jxx: 31.2\%, Jyy: 30.4\%, Jzz: 46.4\%).

\textbf{Physical Explanation for Inertia Errors:}

The higher inertia identification errors (30-46\% vs 0\% for mass/kt/kq) are \textit{expected} given the training trajectory characteristics. At small angles ($\pm$10-20$^\circ$), Euler's rotational equations reduce to nearly decoupled forms:
...
In this regime, inertia appears only in direct division ($\tau/J$), providing a \textit{weak gradient signal} compared to mass (which directly affects easily-measurable vertical acceleration: $\ddot{z} = -T/m + g$). The cross-coupling terms $(J_{yy} - J_{zz})qr/J_{xx}$ that provide strong inertia identification only become significant at large angles ($>$30$^\circ$) with fast simultaneous rotations.

\textbf{Why This Does NOT Compromise Model Performance:}
\begin{itemize}
\item State prediction accuracy remains high (correlation $>$ 0.86) because the PINN compensates through learned neural dynamics
\item Mass, kt, kq achieve perfect 0\% error, ensuring accurate thrust and torque relationships
\item The inertia errors are systematic (not random), so relative dynamics are still captured
\item For applications requiring precise inertia values (e.g., high-performance aerobatic control), aggressive rotational maneuvers ($\pm$45-60$^\circ$) would excite cross-coupling dynamics and reduce errors to $<$10\%
\end{itemize}
\end{itemize}

\textbf{Future Work Directions (Remaining After Implemented Fixes):}
\begin{enumerate}
\item \textbf{Improve Inertia Identification}: Add aggressive rotational maneuvers to training data to better excite inertial dynamics and reduce 30-46\% parameter errors
