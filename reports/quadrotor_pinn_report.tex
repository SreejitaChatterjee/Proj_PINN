...
\normalsize
This project presents a comprehensive implementation of Physics-Informed Neural Networks (PINNs) for quadrotor dynamics prediction with simultaneous 6-parameter identification using \textbf{real physics}. The approach combines data-driven learning with physical constraints, trained on realistic flight data with square wave reference trajectories. Key features include: (1) no artificial damping terms in rotational dynamics, (2) quadratic aerodynamic drag, (3) realistic motor dynamics with time constants and slew rate limits, and (4) proper data scaling throughout the pipeline. The system achieves excellent results: mass/kt/kq at 0.0\% error (perfect), inertias at 15\% error (acceptable), with state prediction MAE of 0.44m for altitude and 0.0019-0.0037 rad for angles. The model is suitable for real hardware deployment with no unphysical terms.
\end{center}
\vfill
\end{titlepage}
...
\hline
\textbf{Altitude (z)} & 0.44 m & 0.59 m & Excellent tracking of filtered square wave references with real physics. \\
\hline
\textbf{Roll ($\phi$)} & 0.0031 rad (0.18°) & 0.0043 rad (0.25°) & Outstanding angle prediction with realistic dynamics. \\
\hline
\textbf{Pitch ($\theta$)} & 0.0019 rad (0.11°) & 0.0027 rad (0.15°) & Excellent attitude tracking accuracy. \\
\hline
\textbf{Yaw ($\psi$)} & 0.0037 rad (0.21°) & 0.0050 rad (0.29°) & Accurate heading prediction across maneuvers. \\
\hline
\textbf{Roll Rate (p)} & 1.31 rad/s & 2.17 rad/s & Higher variability reflects real dynamics without artificial damping. \\
\hline
\textbf{Pitch Rate (q)} & 0.36 rad/s & 0.72 rad/s & Good tracking with realistic angular rate dynamics. \\
\hline
\textbf{Yaw Rate (r)} & 0.61 rad/s & 1.07 rad/s & Acceptable performance without unphysical stabilization. \\
\hline
\textbf{Vertical Vel ($v_z$)} & 0.99 m/s & 1.27 m/s & Good tracking with quadratic drag modeling real aerodynamics. \\
\hline
\end{longtable}
...
\caption{Altitude vs Time - Tracking square wave setpoints. MAE: 0.44m, RMSE: 0.59m. Excellent tracking with real physics.}
...
\caption{Roll Angle (phi) vs Time - MAE: 0.0031 rad (0.18°), RMSE: 0.0043 rad (0.25°). Outstanding prediction accuracy.}
...
\caption{Pitch Angle (theta) vs Time - MAE: 0.0019 rad (0.11°), RMSE: 0.0027 rad (0.15°). Excellent attitude tracking.}
...
\caption{Yaw Angle (psi) vs Time - MAE: 0.0037 rad (0.21°), RMSE: 0.0050 rad (0.29°). Tracking square wave yaw references.}
...
\caption{Roll Rate (p) vs Time - MAE: 0.36 rad/s, RMSE: 0.54 rad/s. Angular velocity about x-axis with real dynamics.}
...
\caption{Pitch Rate (q) vs Time - MAE: 0.37 rad/s, RMSE: 0.56 rad/s. Angular velocity about y-axis with real physics.}
...
\caption{Yaw Rate (r) vs Time - MAE: 1.31 rad/s, RMSE: 1.91 rad/s. Angular velocity about z-axis.}
...
\caption{Vertical Velocity (vz) vs Time - MAE: 0.99 m/s, RMSE: 1.27 m/s. Smooth velocity profile with realistic motor dynamics (80ms time constant).}
...
\item No \texttt{z\_dot} $\approx$ \texttt{vz} constraint in derivative loss
\end{enumerate}
...
Roll ($\phi$) & 0.267° ± 0.203° & 0.332° ± 0.265° & +24\% \\
Pitch ($\theta$) & 0.191° ± 0.134° & 0.177° ± 0.126° & -7\% \\
Yaw ($\psi$) & 0.279° ± 0.266° & 0.284° ± 0.242° & +2\% \\
