...
All visualizations use Trajectory 0 as the representative flight pattern to match MATLAB simulation results. This trajectory was selected for its comprehensive coverage of all control modes and realistic flight dynamics.
...
\caption{Vertical Position (Altitude) vs Time (Trajectory 0) - Climb maneuver from ground level (z=0m) to target altitude of 5.0m (shown as height with sign flip: h=-z). Smooth approach with minimal overshoot demonstrates effective PID altitude control (kz1=2.0, kz2=0.15, kv=-1.0). Red dashed line indicates 5m setpoint. Flight pattern matches MATLAB simulation setpoint $z_r = -5.0$ m.}
