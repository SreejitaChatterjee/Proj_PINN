...
\addlinespace[3pt]
\textbf{2. Trajectory Generation} & Created 10 diverse flight trajectories with square wave reference inputs (roll, pitch, yaw, altitude) and PID controllers generating realistic transient responses. Issue \#6 fix: altitude setpoints limited to realistic range (max 8m) to constrain vertical velocities to $\pm$7 m/s. Square wave periods range from 1.2s to 5.0s with varying amplitudes for rich dynamic behavior & 10 × 5,000 samples = 50,000 comprehensive data points \\
\addlinespace[3pt]
...
\caption{Vertical Position (Altitude) vs Time (Trajectory 0) - Square wave tracking between 3.0m and 5.0m altitude setpoints (period: 2.0s, red dashed lines). The plot shows height (h=-z) to match conventional altitude representation. PID altitude controller (kz1=2.0, kz2=0.15, kv=-1.0 for NED coordinates) demonstrates effective tracking of time-varying reference commands with smooth transitions between setpoint levels.}
...
\caption{Yaw Angle vs Time (Trajectory 0) - Square wave tracking between -5.0° and +5.0° setpoints (period: 3.0s, red dashed lines) demonstrating directional maneuvers. PID controller (k12=1.0, ki2=0.004, k22=0.1) regulates yaw with smooth transitions. Yaw dynamics governed by: $\dot{r} = (J_{xx}-J_{yy})pq/J_{zz} + \tau_z/J_{zz} - 2r$. Independent yaw control during simultaneous roll and pitch maneuvers validates proper implementation of decoupled heading control.}
