...
\hline
\textbf{Shared 1} & 256 & 256 & 65,792 & Nonlinear dynamics modeling (residual) \\
\hline
\textbf{Dropout 2} & 256 & 256 & 0 & Regularization (p=0.1) \\
\hline
\textbf{Shared 2} & 256 & 256 & 65,792 & Deep feature extraction (residual) \\
\hline
\textbf{Dropout 3} & 256 & 256 & 0 & Regularization (p=0.1) \\
\hline
\textbf{Coupling Layer} & \multicolumn{4}{|p{0.68\textwidth}|}{Physics-aware feature separation and merging (131,584 params)} \\
\cline{1-1}
\quad Trans. Branch & 256 & 128 & 32,896 & Translational dynamics (position, velocity) \\
\hline
\quad Rot. Branch & 256 & 128 & 32,896 & Rotational dynamics (angles, rates) \\
\hline
\quad Merge & 256 & 256 & 65,792 & Coupled dynamics integration \\
\hline
...
\hline
\textbf{Total} & - & - & \textbf{268,558} & Complete trainable parameters \\
\hline
\end{longtable}

\textbf{Note:} Enhanced architecture with 256 neurons per layer (vs. 128 previously) and specialized coupling layer for physics-aware feature separation. The coupling layer branches into translational and rotational dynamics, then merges them to preserve physical coupling. Dropout regularization (p=0.1) and residual connections improve autoregressive stability. The network predicts 8 next state variables, with 6 learned physical parameters (m, Jxx, Jyy, Jzz, kt, kq). Gravity (g=9.81 m/s²) is a \textbf{fixed constant} used in physics calculations, NOT a learnable parameter.

\vspace{2em}
\begin{figure}[H]
\centering
\includegraphics[width=1.2\textwidth,page=1]{pinn_schematics.pdf}
\caption{Optimized PINN v2 Architecture: Deep neural network with 256 neurons per layer, specialized coupling layer for physics-aware dynamics separation, dropout regularization, and residual connections. The network takes 12D input (states + controls) and outputs 8D next states, with 6 physics parameters (m, $J_{xx}$, $J_{yy}$, $J_{zz}$, $k_t$, $k_q$) learned during training. Total: 268,558 parameters.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/01_x_time_analysis.png}
\caption{X Position vs Time - Horizontal position in inertial frame computed from body velocities.}
\label{fig:x_position_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/02_y_time_analysis.png}
\caption{Y Position vs Time - Horizontal position in inertial frame computed from body velocities.}
\label{fig:y_position_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/03_z_time_analysis.png}
\caption{Z Position (Altitude) vs Time - MAE: \textbf{0.087m}, RMSE: \textbf{0.174m}. Vertical position tracking by PINN.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/04_roll_time_analysis.png}
\caption{Roll Angle ($\phi$) vs Time - MAE: \textbf{0.0008 rad (0.045°)}, RMSE: \textbf{0.0012 rad}. Outstanding prediction accuracy.}
\label{fig:roll_angle_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/05_pitch_time_analysis.png}
\caption{Pitch Angle ($\theta$) vs Time - MAE: \textbf{0.0005 rad (0.028°)}, RMSE: \textbf{0.0008 rad}. Excellent attitude tracking.}
\label{fig:pitch_angle_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/06_yaw_time_analysis.png}
\caption{Yaw Angle ($\psi$) vs Time - MAE: \textbf{0.0009 rad (0.052°)}, RMSE: \textbf{0.0015 rad}. Accurate heading prediction.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/07_p_time_analysis.png}
\caption{Roll Rate (p) vs Time - MAE: \textbf{0.0029 rad/s}, RMSE: \textbf{0.0047 rad/s}. Smooth predictions with temporal constraints.}
\label{fig:roll_rate_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/08_q_time_analysis.png}
\caption{Pitch Rate (q) vs Time - MAE: \textbf{0.0015 rad/s}, RMSE: \textbf{0.0023 rad/s}. Continuous, physically realistic predictions.}
\label{fig:pitch_rate_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/09_r_time_analysis.png}
\caption{Yaw Rate (r) vs Time - MAE: \textbf{0.0028 rad/s}, RMSE: \textbf{0.0043 rad/s}. Smooth angular velocity tracking.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/10_vx_time_analysis.png}
\caption{X Velocity (u, body frame) vs Time - Horizontal velocity component from translational dynamics.}
\label{fig:vx_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/11_vy_time_analysis.png}
\caption{Y Velocity (v, body frame) vs Time - Horizontal velocity component from translational dynamics.}
\label{fig:vy_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/12_vz_time_analysis.png}
\caption{Z Velocity (w, vertical) vs Time - MAE: \textbf{0.045 m/s}, RMSE: \textbf{0.077 m/s}. Smooth velocity tracking with temporal constraints.}
\label{fig:vertical_velocity_analysis}
\end{figure}

\subsubsection{Angular Accelerations}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/13_p_dot_time_analysis.png}
\caption{Roll Acceleration ($\dot{p}$) vs Time - Angular acceleration measurement used for inertia identification.}
\label{fig:p_dot_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/14_q_dot_time_analysis.png}
\caption{Pitch Acceleration ($\dot{q}$) vs Time - Angular acceleration measurement used for inertia identification.}
\label{fig:q_dot_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/15_r_dot_time_analysis.png}
\caption{Yaw Acceleration ($\dot{r}$) vs Time - Angular acceleration measurement used for inertia identification.}
\label{fig:r_dot_analysis}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/16_thrust_time_analysis.png}
\caption{Total Thrust vs Time - Control input showing realistic motor dynamics with 80ms time constant.}
\label{fig:thrust_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/17_torque_x_time_analysis.png}
\caption{Roll Torque ($\tau_x$) vs Time - Control input for roll attitude maneuvers.}
\label{fig:torque_x_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/18_torque_y_time_analysis.png}
\caption{Pitch Torque ($\tau_y$) vs Time - Control input for pitch attitude maneuvers.}
\label{fig:torque_y_analysis}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/detailed/19_torque_z_time_analysis.png}
\caption{Yaw Torque ($\tau_z$) vs Time - Control input for yaw (heading) maneuvers.}
...
Having established a robust baseline model with strong performance and parameter identification capabilities, we next investigate whether advanced architectural techniques and training methodologies can further improve the model. This section documents a systematic exploration of optimization strategies, analyzing both their benefits and potential pitfalls for autoregressive prediction tasks.
...
Total Parameters & 268,558 & 38,542 (-86\%) \\
...
\begin{enumerate}
\item \textbf{86\% Parameter Reduction}: 38,542 vs 268,558 parameters
  \begin{itemize}
...
Parameters & 268,558 & 37,006 & 35,470 & Optimized (-86\%) \\
Hidden Size & 256 & 128 & 128 & Optimized \\
Training Epochs & 250 & 100 & 100 & Baseline (more stable) \\
Architecture & Coupling-layer & Fourier+Modular & Modular & Baseline (physics-aware) \\
\midrule
...
\textbf{Production model:} Baseline PINN (250 epochs, coupling-layer architecture, 268,558 parameters)
...
\textbf{Result: 51× improvement over baseline (0.029m vs 1.49m at 100-step horizon) with 91.4\% average improvement across all 8 states.}
...
\begin{itemize}
\item \textbf{Parameters:} 268,558 (production baseline model)
\item \textbf{Architecture:} Input + 2 shared layers + coupling layer + output
