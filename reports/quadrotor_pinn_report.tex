...
Individual time-series analysis was performed for all 12 state variables, showing behavior of a representative flight trajectory over a 5-second duration:

\textbf{Control and Position Variables:}
\begin{itemize}
\item Thrust force trajectory demonstrates smooth control transitions
\item Altitude (z-position) shows realistic flight pattern with controlled descent
\item The representative trajectory maintains physical consistency with realistic quadrotor behavior
\end{itemize}
...
\caption{Thrust Force vs Time - Representative flight trajectory showing smooth control transitions and realistic thrust dynamics over 5-second duration.}
...
\caption{Roll Angle vs Time - Banking angle showing smooth attitude control performance in representative flight.}
...
\caption{Pitch Angle vs Time - Nose-up/down attitude demonstrating longitudinal control in representative flight.}
...
\caption{Key Flight Variables Analysis - Critical quadrotor states (altitude, thrust, attitudes, rates) from representative flight trajectory.}
