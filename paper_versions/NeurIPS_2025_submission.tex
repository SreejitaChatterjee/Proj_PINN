...
\begin{enumerate}
    \item We identify the \textbf{expressivity-stability tradeoff} in physics-informed dynamics learning, showing that architectural expressivity inversely correlates with autoregressive stability (Section~\ref{sec:tradeoff}).

    \item We characterize two \textbf{failure mechanisms}: modular architecture decoupling breaks dynamic coupling essential for stable rollout, and Fourier feature embeddings amplify distribution shift exponentially (Section~\ref{sec:failure_modes}).

    \item We propose a \textbf{stability-oriented training framework} combining curriculum learning, scheduled sampling, and physics-consistent regularization, achieving 51$\times$ improvement over baseline (Section~\ref{sec:method}).

    \item We establish that \textbf{single-step metrics are inadequate} for evaluating dynamics models intended for autoregressive deployment, proposing multi-horizon evaluation protocols (Section~\ref{sec:experiments}).
\end{enumerate}
