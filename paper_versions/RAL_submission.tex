...
\begin{enumerate}
    \item \textbf{Stability Envelope Characterization}: We introduce $H_\epsilon$, the maximum horizon where error $<\epsilon$, showing that architectural expressivity shrinks this envelope by $10^2$--$10^6\times$ (Sec.~\ref{sec:failure}).
    \item \textbf{Expressivity-Stability Tradeoff}: We establish the first empirical demonstration of an inverse relationship between single-step accuracy and multi-step stability in PINNs (Sec.~\ref{sec:failure}).
    \item \textbf{Stability-Oriented Training}: A curriculum protocol (horizon scheduling + scheduled sampling + physics regularization) that enlarges $H_\epsilon$ by 51$\times$ (Sec.~\ref{sec:method}).
    \item \textbf{Observability-Limited Identification}: We achieve 0\% error on mass/motor coefficients and match the theoretical 5\% Fisher Information bound for inertias (Sec.~\ref{sec:results}).
    \item \textbf{Physics-Data Conflict Bias}: A new failure mode where excitation degrades identification (5\%$\rightarrow$46\%) due to model mismatch (Sec.~\ref{sec:observability}).
\end{enumerate}
