...
\begin{enumerate}
    ...
    \item \textbf{Failure Mode Analysis}: We identify modular decoupling and Fourier feature extrapolation as key failure mechanisms (Sec.~\ref{sec:failure}).
    \item \textbf{Stability-Oriented Training}: A curriculum protocol that enlarges $H_\epsilon$ by 51$\times$ while maintaining accurate dynamics prediction (Sec.~\ref{sec:method}).
\end{enumerate}
