...
\begin{itemize}
    \item Systematic characterization of autoregressive instability in PINNs, demonstrating that single-step evaluation metrics are insufficient for control applications (Sec.~\ref{sec:failure_modes}).

    \item A curriculum-based training methodology achieving 51$\times$ improvement in 100-step prediction stability while maintaining accurate parameter identification (Sec.~\ref{sec:methodology}).

    \item Experimental validation on 6-DOF quadrotor dynamics with ablation studies quantifying each component's contribution (Sec.~\ref{sec:experiments}).

    \item A negative result showing that aggressive training maneuvers intended to improve parameter observability can degrade identification due to simulator-model mismatch (Sec.~\ref{sec:negative_result}).
\end{itemize}
