...
Training regime---not physics loss itself---is the dominant factor governing autoregressive stability in physics-informed neural networks. In a 6-DOF quadrotor system, we show that naive comparisons between PINNs and pure neural networks are confounded by \textit{early stopping criterion}: when validation loss includes physics terms, models early-stop at suboptimal points for rollout stability. Under fair comparison (supervised-only early stopping), a PINN with physics weight $w=20$ achieves \textbf{38\% better} 100-step stability than the same architecture without physics loss (1.52m vs 2.47m MAE). We introduce the \textit{stability envelope} $H_\epsilon$---the maximum horizon where error remains bounded---as a formal metric linking Lipschitz properties to long-horizon stability. Our ablations reveal that: (1) training regime dominates physics loss effects; (2) physics loss \textit{can help} when training is properly configured; (3) architectural modularity provides additional stability gains independent of physics constraints. These findings clarify when and how physics constraints benefit learned dynamics for control.
...
\textbf{We find that training regime matters more than physics loss.} In systematic experiments on 6-DOF quadrotor dynamics, we demonstrate that naive comparisons between PINNs and pure neural networks are confounded by the \textit{early stopping criterion}. When early stopping is based on total loss (supervised + physics), PINNs appear to underperform. However, under fair comparison with supervised-only early stopping, physics loss \textit{improves} 100-step stability by 38\% (1.52m vs 2.47m MAE).
...
\begin{enumerate}
    ...
    \item \textbf{Fair physics loss evaluation}: Under proper training, physics loss improves stability by 38\% (Sec.~\ref{sec:experiments})
...
\begin{table}[h]
\centering
\caption{Fair Comparison: Physics Weight Sweep (Supervised Early Stop)}
\label{tab:weight_sweep}
\begin{tabular}{lccc}
\toprule
\textbf{$w_{\text{phys}}$} & \textbf{Sup Loss} & \textbf{1-Step MAE} & \textbf{100-Step MAE} \\
\midrule
0.0 & 0.0007 & 0.024m & 2.47m \\
0.1 & 0.0006 & 0.026m & 4.35m \\
1.0 & 0.0013 & 0.029m & 2.55m \\
5.0 & 0.0062 & 0.049m & 6.32m \\
\textbf{20.0} & 0.0063 & 0.041m & \textbf{1.52m} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding}: Under fair comparison, physics loss ($w$=20) achieves \textbf{38\% better} 100-step stability than no physics loss (1.52m vs 2.47m). The relationship is non-monotonic: intermediate weights hurt stability, while high weights help.

\textbf{Explanation}: High physics weight forces the network to satisfy physical constraints strongly, which acts as implicit regularization on the learned dynamics. The total-loss early stopping criterion causes premature termination before the model converges to a stable solution.
...
\begin{enumerate}
    ...
    \item \textbf{Physics loss can help}: Under fair comparison (same early stopping), physics loss ($w$=20) improves 100-step stability by 38\% (1.52m vs 2.47m). The effect is non-monotonic: intermediate weights hurt, high weights help
...
\textbf{Practical recommendations}: For autoregressive control applications, (1) use supervised-only early stopping even when training with physics loss; (2) tune physics weight---high values ($w \geq 20$) can improve stability; (3) consider modular architectures that separate coupled subsystems. Future work includes hardware validation and systematic study of physics weight scheduling.
