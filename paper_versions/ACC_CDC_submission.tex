...
We introduced the stability envelope $H_\epsilon$ as a formal metric for autoregressive stability in learned dynamics models, with theoretical bounds based on Lipschitz analysis. Our experiments on 6-DOF quadrotor dynamics revealed four key findings that challenge prevailing assumptions:
...
\begin{enumerate}
    ...
    \item \textbf{Spectral norm $> 1$ explains instability}: PINN's maximum Jacobian spectral norm (1.35) exceeds the stability threshold, while PureNN stays at the boundary ($\sigma_{\max} \leq 1.00$). This directly explains the 5.8$\times$ stability gap via Theorem~\ref{thm:lipschitz}
    \item \textbf{Robustness under noise/OOD}: PINN exhibits 2.5$\times$ worse OOD performance, confirming physics losses do not improve generalization
\end{enumerate}
