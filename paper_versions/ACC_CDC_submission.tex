...
\begin{proposition}[Gradient Isolation Reduces Lipschitz Constant]
\label{prop:gradient}
In modular training, gradient isolation prevents cross-subsystem weight inflation:
\begin{equation}
\frac{\partial \mathcal{L}_{trans}}{\partial W_{rot}} = 0, \quad \frac{\partial \mathcal{L}_{rot}}{\partial W_{trans}} = 0
\end{equation}
This yields lower cross-coupling and overall Lipschitz constant compared to monolithic training.
\end{proposition}
...
\begin{table}[h]
\centering
\caption{Empirical Lipschitz Constants}
\label{tab:lipschitz}
\begin{tabular}{lcc}
\toprule
\textbf{Architecture} & \textbf{L (max)} & \textbf{Cross-coupling} \\
\midrule
Baseline & 1.52 & 0.62 \\
\textbf{Modular} & \textbf{1.15} & \textbf{0.17} \\
Fourier & 6.45 & 1.59 \\
\bottomrule
\end{tabular}
\end{table}

The modular architecture achieves 24\% lower Lipschitz constant and 3.7$\times$ lower cross-coupling, directly explaining its superior autoregressive stability.
