...
\begin{enumerate}
    ...
    \item \textbf{Analytical Derivation}: We derive closed-form bias expressions for a 1D toy system, showing $\hat{c} \approx 2c \cdot \mathbb{E}[v]$ when quadratic drag is fit with linear model (Sec.~\ref{sec:mismatch}).
    \item \textbf{Experimental Validation}: Aggressive trajectories ($\pm 60^{\circ}$) increase inertia error from 5\% to 46\%, validating the excitation paradox (Sec.~\ref{sec:mismatch}).
\end{enumerate}
