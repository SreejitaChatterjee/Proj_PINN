...
\title{\LARGE \bf
Observability Limits of PINN-Based System Identification:\\Fisher Information Bounds and the Excitation Paradox
}
...
Physics-Informed Neural Networks (PINNs) enable simultaneous dynamics learning and parameter identification by embedding governing equations into neural network training. We present a systematic analysis of the \textit{observability limits} of PINN-based system identification for 6-DOF quadrotor dynamics. Using Fisher Information analysis, we explain why mass and motor coefficients achieve 0\% identification error while inertia parameters saturate at 5\%---matching theoretical observability bounds. Critically, we identify a \textit{physics-data conflict bias}: when the PINN's physics model is incomplete, increased trajectory excitation paradoxically \textit{degrades} parameter identification (5\%$\rightarrow$46\% error) rather than improving it. This occurs because the network learns biased ``effective parameters'' that absorb missing dynamics (e.g., nonlinear aerodynamics). We prove this bias analytically for a 1D toy system and validate it experimentally with aggressive quadrotor maneuvers ($\pm 45$--$60^{\circ}$). These results establish that excitation design must match model fidelity---a fundamental constraint for PINN-based system identification in robotics.
...
System identification is fundamental to model-based control. Physics-Informed Neural Networks (PINNs) embed governing equations into neural network training~\cite{raissi2019physics}, enabling simultaneous dynamics learning and parameter identification. However, a critical question remains underexplored: \textit{what are the fundamental limits of PINN-based parameter identification?}

We address this through two contributions. First, we characterize \textit{observability limits} using Fisher Information: parameters with strong gradients in the dynamics (mass, motor coefficients) achieve near-zero error, while parameters with weak coupling (inertias at small angles) saturate at theoretical bounds. Second, we identify a novel failure mode---\textit{physics-data conflict bias}---where increased excitation \textit{degrades} rather than improves identification when the physics model is incomplete.

The contributions of this paper are:
\begin{enumerate}
    \item \textbf{Fisher Information Characterization}: We analyze observability limits explaining differential identification accuracy: 0\% error for mass/motor vs 5\% saturation for inertias (Sec.~\ref{sec:observability}).
    \item \textbf{Physics-Data Conflict Bias}: We prove that PINNs with incomplete physics learn biased ``effective parameters'' that absorb missing dynamics. Bias \textit{increases} with excitation---contradicting classical system identification intuition (Sec.~\ref{sec:mismatch}).
    \item \textbf{Analytical Derivation}: We derive closed-form bias expressions for a 1D toy system, showing $\hat{c} \approx 2c \cdot \mathbb{E}[v]$ when quadratic drag is fit with linear model (Sec.~\ref{sec:theory}).
    \item \textbf{Experimental Validation}: Aggressive trajectories ($\pm 60^{\circ}$) increase inertia error from 5\% to 46\%, validating the excitation paradox (Sec.~\ref{sec:experiments}).
\end{enumerate}
