...
\textbf{Statistical analysis}: Welch's $t$-test yields $t=1.68$, $p=0.103$, Cohen's $d=0.53$ (medium effect). The difference is \textbf{not statistically significant} at $\alpha=0.05$. Modular shows consistently better single-step accuracy (0.023 vs 0.045) with much lower variance ($\pm$0.001 vs $\pm$0.010), and a trend toward better rollout stability, but we cannot claim statistical significance for the rollout difference.
...
\begin{enumerate}
    ...
    \item \textbf{Physics loss observation}: In our specific setup (w=20, lr=$10^{-3}$, 20 seeds), w=0 achieved 1.74$\pm$1.03m vs w=20 achieved 2.72$\pm$1.54m ($p=0.024$, $d=0.75$). This does \textbf{not} imply physics loss is generally harmful---it highlights the need for careful tuning
    \item \textbf{Architecture observation}: Modular architecture achieved 1.96$\pm$0.99m vs Baseline 2.65$\pm$1.55m, but this difference is \textbf{not statistically significant} ($p=0.103$). Modular shows lower variance and a trend toward better performance
