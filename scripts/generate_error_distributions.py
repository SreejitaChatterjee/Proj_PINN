...
def plot_error_distributions(errors, save_dir):
    ...
    state_names = ['x', 'y', 'z', 'roll', 'pitch', 'yaw', 'p', 'q', 'r', 'vx', 'vy', 'vz']
    ...
    for i, (name, unit) in enumerate(zip(state_names, state_units)):
        ...
        ax = axes[0]
        n, bins, patches = ax.hist(error, bins=50, density=True, alpha=0.7, color='steelblue', edgecolor='black')

        # Fit and plot normal distribution
        mu, sigma = error.mean(), error.std()
        x = np.linspace(error.min(), error.max(), 100)
        ax.plot(x, stats.norm.pdf(x, mu, sigma), 'r-', linewidth=2, label=f'Normal({mu:.2e}, {sigma:.2e})')
        ...
        _, p_value = stats.shapiro(error[:5000] if len(error) > 5000 else error)  # Shapiro-Wilk test
        ax.text(0.02, 0.98, f'Shapiro-Wilk p-value: {p_value:.4f}\n' +
...
def plot_combined_error_overview(errors, save_dir):
    ...
    for i, (name, unit) in enumerate(zip(state_names, state_units)):
        ...
        # Histogram
        ax.hist(error, bins=40, density=True, alpha=0.7, color='steelblue', edgecolor='black')
    ...
    plt.suptitle('Prediction Error Distributions - All States', fontsize=16, fontweight='bold', y=0.995)
    plt.tight_layout()
