...
def main():
    ...
    print(f"Standard training data: {len(df_standard)} samples, {df_standard['trajectory_id'].nunique()} trajectories")

    # Load aggressive trajectories
    df_aggressive = pd.read_csv(PROJECT_ROOT / 'data' / 'aggressive_inertia_trajectories.csv')
    print(f"Aggressive trajectory data: {len(df_aggressive)} samples, {df_aggressive['trajectory_id'].nunique()} trajectories")
