...
# EuRoC sequences (all 11 sequences, easiest to hardest)
SEQUENCES = {
    # Machine Hall
...
def load_imu(seq_dir):
    ...
    df = pd.read_csv(imu_path, header=0, names=["timestamp", "wx", "wy", "wz", "ax", "ay", "az"])
...
def prepare_dynamics_data(seq_dir, dt=0.005):
    ...
    eulers = np.array([quat_to_euler(row.qw, row.qx, row.qy, row.qz) for _, row in gt.iterrows()])
    gt["roll"] = eulers[:, 0]
    gt["pitch"] = eulers[:, 1]
    gt["yaw"] = eulers[:, 2]
