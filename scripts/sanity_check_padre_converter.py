...
def test_hover_physics():
    ...
    all_passed &= test_result(
        "Roll near zero in hover",
        phi_mean < 0.05,
        f"Mean |phi| = {phi_mean:.4f} rad"
    )

    all_passed &= test_result(
        "Pitch near zero in hover",
        theta_mean < 0.05,
        f"Mean |theta| = {theta_mean:.4f} rad"
    )

    # Angular rates should be near zero
    pqr_mean = np.abs(states[:, 6:9]).mean()
    all_passed &= test_result(
        "Angular rates near zero in hover",
        pqr_mean < 0.05,
        f"Mean |p,q,r| = {pqr_mean:.4f} rad/s"
    )
    ...
    all_passed &= test_result(
        "Torques near zero in hover",
        torques_mean < 0.01,
        f"Mean |torques| = {torques_mean:.6f} Nm"
    )
...
def test_windowed_output():
    ...
    all_passed &= test_result(
        "X last dim is 16 (states + controls)",
        X.shape[-1] == 16
    )

    all_passed &= test_result(
        "Y last dim is 12 (states only)",
        Y.shape[-1] == 12
    )
...
def test_physics_consistency():
    ...
    try:
        ...
        pinn = QuadrotorPINN()
        converter = PADREtoPINNConverter(
            mass=0.068,  # Match PINN defaults
            Jxx=6.86e-5,
            Jyy=9.2e-5,
            Jzz=1.366e-4
        )
...
def test_state_ranges():
    ...
    all_passed &= test_result(
        "Thrust positive",
        thrust_min >= 0,
        f"Min thrust = {thrust_min:.4f}N"
    )

    all_passed &= test_result(
        "Thrust bounded (< 50N)",
        thrust_max < 50,
        f"Max thrust = {thrust_max:.4f}N"
    )
...
def test_motor_asymmetry():
    ...
    all_passed &= test_result(
        "Handles asymmetric motor noise",
        not np.any(np.isnan(result['states'])),
        "NaN in output"
    )
