"""Evaluation script for optimized PINN model"""
import torch
import pandas as pd
import numpy as np
import joblib
...
def evaluate_model(model_path, scaler_path, data_path, device='cpu'):
    """Evaluate optimized PINN model on test data"""

    # Load model (must match training config - Phase 1: num_fourier_freq=1)
    ...
    print("="*70)
    print("OPTIMIZED PINN EVALUATION")
    print("="*70)
    print(f"Model: Fourier Features + Residual MLP + Modular Architecture")
    print(f"Architecture: ~50% fewer parameters than baseline")
    print()

    # 1. Teacher-Forced Evaluation (single-step)
    ...
    for t in range(100):
        # Prepare input: [state(8) + control(4)]
    ...
    time = np.arange(101) * 0.001  # 1ms timesteps

    fig, axes = plt.subplots(4, 2, figsize=(14, 12))
    fig.suptitle('Optimized PINN - Autoregressive Rollout (100 steps, 0.1s)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plot_path = results_dir / 'optimized_pinn_evaluation.png'
...
if __name__ == "__main__":
    model_path = Path(__file__).parent.parent / 'models' / 'quadrotor_pinn_optimized.pth'
    scaler_path = Path(__file__).parent.parent / 'models' / 'scalers_optimized.pkl'
