...
def detect_anomalies(tex_file):
    ...
    if inconsistent_spacing:
        issues.append(f"FORMATTING: Found {len(inconsistent_spacing)} lines with inconsistent operator spacing")
    ...
    if decimals:
        ...
        if len(precision_counter) > 2:
            issues.append(f"INCONSISTENCY: Varying decimal precision in percentages: {dict(precision_counter)}")
    ...
    issues.append(f"INFO: Math usage - Inline: {inline_math}, Display: {display_math}, Equations: {equation_envs}")
    ...
    issues.append(f"INFO: Document structure - Sections: {sections}, Subsections: {subsections}, Subsubsections: {subsubsections}")
...
def main():
    ...
    for issue in issues:
        if issue.startswith("WARNING"):
            print(f"[!] {issue}")
        elif issue.startswith("CONTENT") or issue.startswith("FORMATTING") or issue.startswith("INCONSISTENCY"):
            print(f"[X] {issue}")
        else:
            print(f"[i] {issue}")
