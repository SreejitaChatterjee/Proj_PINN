...
def train_model(model, train_loader, val_loader, scaler_X, scaler_y, model_name,
                use_physics=True, seed=0):
...
def main():
    ...
    for name, use_physics in [('PureNN', False), ('PINN_Baseline', True)]:
        ...
        model = train_model(model, train_loader, val_loader, scaler_X, scaler_y,
                           name, use_physics=use_physics, seed=42)
    ...
    for name, ModelClass in [('SmallBaseline_72K', SmallBaseline), ('Modular_72K', ModularPINN)]:
        print(f"\n  Training {name}...")
        model = ModelClass()
    ...
    for arch_name, ModelClass in [('Baseline', BaselinePINN), ('Modular', ModularPINN)]:
        ...
        for seed in seeds:
            ...
            model = ModelClass()
            model = train_model(model, train_loader, val_loader, scaler_X, scaler_y,
                               f'{arch_name}', use_physics=True, seed=seed)
    ...
    ls_results = least_squares_baseline(X_train, y_train, X_val, y_val,
                                        val_trajectories, scaler_X, scaler_y)
    ...
    for name in ['PureNN', 'PINN_Baseline']:
        r = all_results[name]
        print(f"{name:<20} {r['n_params']:<10,} {r['single_step']['z_mae']:<12.4f} {r['rollout']['position_mae']:<12.4f}")
    ...
    print("-"*50)
    print(f"{'LeastSquares':<20} {all_results['LeastSquares']['single_step']['z_mae']:<12.4f} {all_results['LeastSquares']['rollout']['position_mae']:<12.4f}")
    print(f"{'Modular PINN':<20} {all_results['Modular_72K']['single_step']['z_mae']:<12.4f} {all_results['Modular_72K']['rollout']['position_mae']:<12.4f}")
