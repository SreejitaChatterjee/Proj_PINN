...
def generate_stability_figure(arch_results):
    ...
    bars1 = ax1.bar(x - width/2, single_step_z, width, label='Position (m)', color=colors, alpha=0.8)
    ...
    for bar, val in zip(bars2, hundred_step_pos):
        if val > 0:
            ...
            ax2.annotate(f'{val:.2f}' if val < 100 else f'{val:.0e}',
...
def generate_ablation_figure(ablation_results):
    ...
    config_labels = ['Baseline', '+Curriculum', '+Sched.\nSampling', '+Dropout', '+Energy\nCons.']
...
"""
    ...
    configs = ['Baseline', '+Curriculum', '+Sched_Sampling', '+Dropout', '+Energy_Cons']
    config_labels = ['Baseline', '+ Curriculum', '+ Scheduled sampling', '+ Dropout', '+ Energy conservation']
