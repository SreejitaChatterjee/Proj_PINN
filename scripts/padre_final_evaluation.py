...
def main():
    ...
    results = {"timestamp": datetime.now().isoformat(), "evaluations": {}}
    ...
    for drone, data_dir in [("Bebop", bebop_dir), ("Solo", solo_dir)]:
        for csv_file in sorted(data_dir.glob("*.csv")):
            ...
            print(
                f"{drone:7s} {codes:6s} {actual:7s} {pred:9s}  {fault_type:14s} {motor:5s}  {status}"
            )

    total = sum(physics_results.values())
    physics_accuracy = (physics_results["tp"] + physics_results["tn"]) / total
    physics_normal_acc = (
        physics_results["tn"] / (physics_results["tn"] + physics_results["fp"])
        if (physics_results["tn"] + physics_results["fp"]) > 0
        else 0
    )
    physics_faulty_acc = (
        physics_results["tp"] / (physics_results["tp"] + physics_results["fn"])
        if (physics_results["tp"] + physics_results["fn"]) > 0
        else 0
    )
    ...
    print(
        f"  Normal Accuracy: {physics_normal_acc*100:.1f}% (TN={physics_results['tn']}, FP={physics_results['fp']})"
    )
    print(
        f"  Faulty Accuracy: {physics_faulty_acc*100:.1f}% (TP={physics_results['tp']}, FN={physics_results['fn']})"
    )
    ...
    print("+----------------------------------+----------+----------+----------+")
    print(
        f"| Physics-Based Cross-Drone        | {physics_accuracy*100:6.1f}%  | {physics_normal_acc*100:6.1f}%  | {physics_faulty_acc*100:6.1f}%  |"
    )
    print(
        f"| ML Temporal Split (same-drone)   | {ml_temporal_accuracy*100:6.1f}%  | {ml_temporal_normal_acc*100:6.1f}%  | {ml_temporal_faulty_acc*100:6.1f}%  |"
    )
    print(
        f"| ML Cross-Drone (Bebop->Solo)     | {b2s_accuracy*100:6.1f}%  | {b2s_normal_acc*100:6.1f}%  | {b2s_faulty_acc*100:6.1f}%  |"
    )
    print(
        f"| ML Cross-Drone (Solo->Bebop)     | {s2b_accuracy*100:6.1f}%  | {s2b_normal_acc*100:6.1f}%  | {s2b_faulty_acc*100:6.1f}%  |"
    )
    print("+----------------------------------+----------+----------+----------+")
