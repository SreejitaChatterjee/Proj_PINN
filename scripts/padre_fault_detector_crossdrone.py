...
class CrossDroneFaultDetector:
    ...
    def __init__(self, dominance_threshold=0.71,
...
def main():
    ...
    print("\nResults:")
    print("Drone   Code   Actual  Predicted  Rule           Motor  Dominance  Entropy  MaxDev  Status")
    print("-" * 100)
    ...
    for drone, data_dir in [('Bebop', bebop_dir), ('Solo', solo_dir)]:
        for csv_file in sorted(data_dir.glob('*.csv')):
            ...
            print(f"{drone:7s} {codes:6s} {actual:7s} {pred:9s}  {fault_type:14s} {motor:5s}  "
    ...
    normal_acc = results['tn'] / (results['tn'] + results['fp']) * 100 if (results['tn'] + results['fp']) > 0 else 0
    faulty_acc = results['tp'] / (results['tp'] + results['fn']) * 100 if (results['tp'] + results['fn']) > 0 else 0
