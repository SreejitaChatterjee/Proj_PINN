"""Optimized Physics-Informed Neural Network for Quadrotor Dynamics

Key Improvements:
- Fourier feature encoding for periodic states
- Residual MLP layers with Swish activation
- Modular architecture (translational + rotational)
- Energy-based physics constraints
- Reduced parameter count (256 -> 128 neurons)
"""
import torch
...
class FourierFeatures(nn.Module):
    ...
    def __init__(self, input_dim, num_frequencies=4):
        ...
        # Output: input + sin/cos for each frequency = input_dim * (1 + 2*num_frequencies)
    ...
    def forward(self, x):
        ...
        Returns:
            encoded: (batch, input_dim * (1 + 2*num_frequencies))
        """
        batch_size = x.shape[0]
        encoded = [x]  # Include original features

        for k in range(1, self.num_frequencies + 1):
            encoded.append(torch.sin(k * math.pi * x))
            encoded.append(torch.cos(k * math.pi * x))
...
class QuadrotorPINNOptimized(nn.Module):
    def __init__(self, hidden_size=128, dropout=0.1, num_fourier_freq=3):
        super().__init__()

        # Fourier feature encoding for periodic states
        # States: [z, phi, theta, psi, p, q, r, vz]
        # Apply Fourier to angles and rates: [phi, theta, psi, p, q, r]
        self.fourier_periodic = FourierFeatures(6, num_fourier_freq)  # For periodic states

        # Input: z(1) + vz(1) + fourier(6) + controls(4)
        fourier_dim = self.fourier_periodic.output_dim
        total_input_dim = 2 + fourier_dim + 4  # z, vz, fourier(phi,theta,psi,p,q,r), controls

        # Modular architecture
        self.translational = TranslationalModule(total_input_dim, hidden_size // 2)
        self.rotational = RotationalModule(total_input_dim, hidden_size // 2)
    ...
    def forward(self, x):
        ...
        z = x[:, 0:1]
        vz = x[:, 7:8]
        periodic_states = x[:, 1:7]  # phi, theta, psi, p, q, r
        controls = x[:, 8:12]

        # Apply Fourier encoding to periodic states
        periodic_encoded = self.fourier_periodic(periodic_states)

        # Concatenate all features
        features = torch.cat([z, vz, periodic_encoded, controls], dim=1)
    ...
    def energy_loss(self, inputs, outputs):
        ...
        Total energy: E = 0.5*m*vz^2 + 0.5*omega^T*J*omega + m*g*z
        """
        # Current state
        ...
        # Work done by thrust (simplified)
        work = thrust * (z_next - z)  # Thrust * displacement

        # Penalize energy violations (allow some dissipation for damping)
    ...
    def physics_loss(self, inputs, outputs, dt=0.001):
        """Physics-based residual loss with normalized equations"""
        z, phi, theta, psi, p, q, r, vz = inputs[:, :8].T
        ...
        # Vertical dynamics with quadratic drag
        drag_coeff = 0.05
    ...
    def multistep_rollout_loss(self, inputs, num_steps=3):
        ...
        Helps reduce error accumulation in long rollouts.

        Args:
            inputs: (batch, 12) - initial [states(8) + controls(4)]
