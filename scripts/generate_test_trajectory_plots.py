"""Generate comparative plots showing TEST SET trajectories ONLY"""
import torch
import pandas as pd
import numpy as np
...
TEST_DATA_PATH = PROJECT_ROOT / 'data' / 'test_set.csv'
OUTPUT_DIR = PROJECT_ROOT / 'results' / 'test_set_trajectories_fixed'
...
def main():
    print("="*80)
    print("GENERATING TEST SET TRAJECTORY PLOTS")
    print("(HELD-OUT DATA - NOT SEEN DURING TRAINING)")
    print("="*80)

    # Check if test data exists
    if not TEST_DATA_PATH.exists():
        print(f"\n[ERROR] Test data not found at: {TEST_DATA_PATH}")
        print("Please run 'split_train_test_data.py' first to create train/test split")
        return
    ...
    # Load scalers
    scaler_path = MODEL_PATH.parent / 'scalers_fixed.pkl'
    ...
    print(f"\nGenerating test set plots ({nrows}×{ncols} grid)...")
    plot_num = 1
    overall_metrics = {}
    ...
    for var_name, var_label, var_unit, var_type in VARIABLES:
        ...
        if is_predicted and len(all_errors) > 0:
            ...
            title = f'TEST SET: {var_label} - Overall MAE: {overall_mae:.4f} {var_unit}'
        else:
            title = f'TEST SET: {var_label}'
        ...
        # Save plot
        output_file = OUTPUT_DIR / f'{plot_num:02d}_{var_name}_test_trajectories.png'
    ...
    print(f"{'='*80}")
    print(f"Number of test trajectories: {len(trajectory_data)}")
    print(f"Test trajectory IDs: {list(trajectory_data.keys())}")
    print(f"\nOverall MAE per variable:")
    ...
    print(f"\n{'='*80}")
    print(f"SUCCESS: Generated {plot_num-1} test set plots")
    print(f"  Output directory: {OUTPUT_DIR}")
    print(f"  [NOTE] These are HELD-OUT test trajectories (not seen during training)")
    print(f"{'='*80}")
