...
def main():
    ...
    for i, config in enumerate(configs):
        if (i + 1) % 10 == 0:
            print(
                f"  Progress: {i+1}/{n_trajectories} trajectories ({(i+1)/n_trajectories*100:.0f}%)"
            )
    ...
    print(
        f"Roll amplitude range: {roll_range.min()*180/np.pi:.1f}° to {roll_range.max()*180/np.pi:.1f}°"
    )
    print(
        f"Pitch amplitude range: {pitch_range.min()*180/np.pi:.1f}° to {pitch_range.max()*180/np.pi:.1f}°"
    )
    print(
        f"Yaw amplitude range: {yaw_range.min()*180/np.pi:.1f}° to {yaw_range.max()*180/np.pi:.1f}°"
    )
