...
def plot_jacobian_histograms(models, scaler_X, n_samples=500):
    ...
    for idx, (model_name, model) in enumerate(models.items()):
        ...
        print(
            f"  Mean: {np.mean(sigmas):.4f}, P95: {np.percentile(sigmas, 95):.4f}, Max: {np.max(sigmas):.4f}"
        )
    ...
    for model_name, sigmas in all_sigmas.items():
        ax.hist(
            ...
            bins=50,
            alpha=0.5,
            label=f"{model_name} (μ={np.mean(sigmas):.2f})",
            color=colors.get(model_name, "gray"),
        )
...
def main():
    ...
    results = {
        "robustness": {
            k: {kk: float(vv) for kk, vv in v.items()} for k, v in robustness_results.items()
        },
        "jacobian_stats": {
            k: {
            ...
            for k, v in jacobian_results.items()
        },
    }
    ...
    for model_name in models.keys():
        ...
        print(
            f"{model_name:<12} {r['Clean']:<10.3f} {r['Noise_5pct']:<10.3f} {r['Dropout_10pct']:<10.3f} {r['Both']:<10.3f}"
        )
    ...
    for model_name, sigmas in jacobian_results.items():
        print(
            f"{model_name:<12} {np.mean(sigmas):<10.3f} {np.percentile(sigmas, 95):<10.3f} {np.max(sigmas):<10.3f}"
        )
