...
cohens_d = (np.mean(w20_rollouts) - np.mean(w0_rollouts)) / np.sqrt(
    (np.std(w0_rollouts) ** 2 + np.std(w20_rollouts) ** 2) / 2
)
...
high_loss_seeds_w0 = [
    (i, s["seed"], s["sup_loss"], s["rollout_mae"])
...
if high_loss_w0:
    ...
    print(
        f"  High-loss group (n={len(high_loss_w0)}): rollout = {np.mean([s['rollout_mae'] for s in high_loss_w0]):.2f}m"
    )
...
d_arch = (np.mean(baseline_rollouts) - np.mean(modular_rollouts)) / np.sqrt(
    (np.std(baseline_rollouts) ** 2 + np.std(modular_rollouts) ** 2) / 2
)
...
print(
    f"      Variance ratio: Baseline has {np.std(baseline_single)/np.std(modular_single):.1f}x higher std!"
)
...
print(
    f"w=20 seeds beating w=0 median ({np.median(w0_rollouts):.2f}m): {w20_wins}/20 ({100*w20_wins/20:.0f}%)"
)
...
for name, data in [
    ("w=0 rollout", w0_rollouts),
    ("w=20 rollout", w20_rollouts),
    ("Baseline rollout", baseline_rollouts),
    ("Modular rollout", modular_rollouts),
]:
    ...
    print(f"\n{name}:")
    print(
        f"  Shapiro-Wilk: W = {shapiro_stat:.3f}, p = {shapiro_p:.4f} ({'Normal' if shapiro_p > 0.05 else 'Non-normal'})"
    )
    print(
        f"  Skewness: {skew:.2f} ({'right-skewed' if skew > 0.5 else 'left-skewed' if skew < -0.5 else 'symmetric'})"
    )
    print(
        f"  Kurtosis: {kurt:.2f} ({'heavy-tailed' if kurt > 1 else 'light-tailed' if kurt < -1 else 'normal-tailed'})"
    )
