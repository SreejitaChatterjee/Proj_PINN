...
for csv_file in sorted(data_dir.glob('*.csv')):
    ...
    for i in range((len(data) - 256) // 128 + 1):
        ...
        feat = []
        for col in range(24):
            feat.extend([window[:, col].mean(), window[:, col].std(), window[:, col].max() - window[:, col].min()])
        X.append(feat)
...
rf2 = RandomForestClassifier(n_estimators=100, class_weight='balanced', random_state=42, n_jobs=-1).fit(X_train, y_bin_train)
rf3 = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1).fit(X_train, y_mot_train)
rf4 = RandomForestClassifier(n_estimators=100, class_weight='balanced', random_state=42, n_jobs=-1).fit(X_train, y_mot_train)
rf5 = MultiOutputClassifier(RandomForestClassifier(n_estimators=100, class_weight='balanced', random_state=42, n_jobs=-1)).fit(X_train, y_multi_train)

p1, p2, p3, p4, p5 = rf1.predict(X_test), rf2.predict(X_test), rf3.predict(X_test), rf4.predict(X_test), rf5.predict(X_test)
...
print(f'Accuracy:    {accuracy_score(y_bin_test, p1):.4f} ({accuracy_score(y_bin_test, p1) * 100:.2f}%)')
...
tn, fp, fn, tp = cm.ravel()
print(f'Accuracy:    {accuracy_score(y_bin_test, p2):.4f} ({accuracy_score(y_bin_test, p2) * 100:.2f}%)')
...
print(header3)
print('-' * 88)
print(f'{"Binary (Unbalanced)":<40} {accuracy_score(y_bin_test, p1):<12.4f} {f1_score(y_bin_test, p1):<12.4f} {precision_score(y_bin_test, p1):<12.4f} {recall_score(y_bin_test, p1):<12.4f}')
print(f'{"Binary (Balanced)":<40} {accuracy_score(y_bin_test, p2):<12.4f} {f1_score(y_bin_test, p2):<12.4f} {precision_score(y_bin_test, p2):<12.4f} {recall_score(y_bin_test, p2):<12.4f}')
print(f'{"Motor ID (Unbalanced)":<40} {accuracy_score(y_mot_test, p3):<12.4f} {f1_score(y_mot_test, p3, average="weighted"):<12.4f} {precision_score(y_mot_test, p3, average="weighted"):<12.4f} {recall_score(y_mot_test, p3, average="weighted"):<12.4f}')
print(f'{"Motor ID (Balanced)":<40} {accuracy_score(y_mot_test, p4):<12.4f} {f1_score(y_mot_test, p4, average="weighted"):<12.4f} {precision_score(y_mot_test, p4, average="weighted"):<12.4f} {recall_score(y_mot_test, p4, average="weighted"):<12.4f}')
