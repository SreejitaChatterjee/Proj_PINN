...
def create_summary_figure(pinn_overall, per_fault, baselines, output_dir: Path):
    ...
    f1 = [pinn_overall['mean_f1']*100, baselines['SVM']['f1']*100,
          baselines['IForest']['f1']*100, baselines['Chi2']['f1']*100]
    fpr = [pinn_overall['mean_fpr']*100, baselines['SVM']['fpr']*100,
           baselines['IForest']['fpr']*100, baselines['Chi2']['fpr']*100]

    x = np.arange(len(methods))
    width = 0.35

    bars1 = ax1.bar(x - width/2, f1, width, label='F1 Score', alpha=0.8, color='#2E86AB')
    bars2 = ax1.bar(x + width/2, fpr, width, label='False Positive Rate', alpha=0.8, color='#C73E1D')
    ...
    prec = [pinn_overall['mean_precision']*100, baselines['SVM']['precision']*100,
            baselines['IForest']['precision']*100, baselines['Chi2']['precision']*100]
    rec = [pinn_overall['mean_recall']*100, baselines['SVM']['recall']*100,
           baselines['IForest']['recall']*100, baselines['Chi2']['recall']*100]

    colors = ['#2E86AB', '#A23B72', '#F18F01', '#C73E1D']
    for i, (p, r, m) in enumerate(zip(prec, rec, methods)):
        ax2.scatter(r, p, s=200, alpha=0.7, color=colors[i], edgecolors='black', linewidth=1.5, label=m)
    ...
    short_names = [ft.replace('_', '\n').replace('Stuck', 'Stuck\n').replace('Failure', 'Fail.') for ft in fault_types]
    ...
    plt.suptitle('PINN-Based Fault Detection: Comprehensive Evaluation', fontsize=16, fontweight='bold', y=0.98)
