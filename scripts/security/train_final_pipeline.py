...
for mag in [0.3, 0.5, 0.7, 1.0, 1.3, 1.5, 2.0, 2.5]:
    ...
    for attack_type in ["drift", "bias", "noise", "jump"]:
        if attack_type == "drift":
            attack = attack_base + np.linspace(0, 5 * mag, 1000).reshape(-1, 1) * np.array(
                [1, 1, 0.5, 0, 0, 0, 0, 0, 0, 0, 0, 0]
            )
        elif attack_type == "bias":
            attack = attack_base + np.array(
...
for attack_name, attack_data in test_attacks.items():
    ...
    print(
        f"{attack_name:<25} | {np.mean(iso_pred)*100:>9.1f}% | {np.mean(clf_pred)*100:>9.1f}% | {np.mean(ens_pred)*100:>9.1f}%"
    )
...
best_method = max(
    ["iso_forest", "classifier", "ensemble"],
