...
for contamination in [0.01, 0.02, 0.03, 0.05, 0.07, 0.10, 0.15]:
    ...
    print(f'  contamination={contamination:.2f}: Recall={recall*100:5.1f}%, FPR={fpr*100:5.1f}%, Score={score*100:.1f}')
...
for windows in window_configs:
    ...
    recall = np.mean(recalls)
    score = recall - 0.5*fpr
    print(f'  windows={windows}: Recall={recall*100:5.1f}%, FPR={fpr*100:5.1f}%, Score={score*100:.1f}')
...
for contamination in [0.02, 0.03, 0.05, 0.07]:
    ...
    recall = np.mean(recalls)
    score = recall - 0.5*fpr
    print(f'  Enhanced (c={contamination:.2f}): Recall={recall*100:5.1f}%, FPR={fpr*100:5.1f}%, Score={score*100:.1f}')
...
for kalman_pct in [95, 97, 99]:
    ...
    for vote_thresh in [1, 2]:
        ...
        print(f'  Kalman({kalman_pct})+MultiScale ({mode}): Recall={recall*100:5.1f}%, FPR={fpr*100:5.1f}%, Score={score*100:.1f}')
...
for n_sigma in [2.0, 2.5, 3.0, 3.5, 4.0, 5.0]:
    ...
    recall = np.mean(recalls)
    score = recall - 0.5*fpr
    print(f'  n_sigma={n_sigma:.1f}: Recall={recall*100:5.1f}%, FPR={fpr*100:5.1f}%, Score={score*100:.1f}')
...
for method, config, recall, fpr, score in all_results[:15]:
    ...
    print(f'{method:<25} {config:<25} {recall*100:>7.1f}% {fpr*100:>7.1f}% {score*100:>7.1f} {status}')
