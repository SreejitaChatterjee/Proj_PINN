...
def plot_score_distributions(scores, labels, fault_types, output_dir: Path):
    ...
    axes[0].hist(normal_scores, bins=50, alpha=0.6, label=f'Normal (n={len(normal_scores)})', density=True)
    axes[0].hist(fault_scores, bins=50, alpha=0.6, label=f'Fault (n={len(fault_scores)})', density=True)
