...
def plot_residuals_vs_time(errors, timestamps, save_dir):
    ...
    state_units = ['m', 'm', 'm', 'rad', 'rad', 'rad', 'rad/s', 'rad/s', 'rad/s', 'm/s', 'm/s', 'm/s']
    ...
    for i, (name, unit) in enumerate(zip(state_names, state_units)):
        ...
        if len(error) > window:
            ...
            ax.plot(timestamps, running_mean, color='orange', linewidth=2, label=f'Running mean ({window} samples)')
    ...
    plt.suptitle('Residuals vs Time - Temporal Error Patterns', fontsize=16, fontweight='bold', y=0.995)
    plt.tight_layout()
...
def plot_residuals_vs_predicted(errors, predictions, save_dir):
    ...
    for i, (name, unit) in enumerate(zip(state_names, state_units)):
        ...
        if len(pred) > 10:
            ...
            ax.plot(x_trend, p(x_trend), color='orange', linewidth=2, linestyle='--',
    ...
    plt.suptitle('Residuals vs Predicted Values - Heteroscedasticity Check', fontsize=16, fontweight='bold', y=0.995)
    plt.tight_layout()
...
def plot_autocorrelation(errors, save_dir):
    ...
    for i, name in enumerate(state_names):
        ...
        lags = range(max_lag)
        acf = [np.corrcoef(error[:-lag or None], error[lag:])[0, 1] if lag > 0
    ...
    plt.suptitle('Residual Autocorrelation - Temporal Dependencies', fontsize=16, fontweight='bold', y=0.995)
    plt.tight_layout()
...
def plot_rolling_statistics(errors, timestamps, save_dir):
    ...
    for i, (name, unit) in enumerate(zip(state_names, state_units)):
        ...
        if len(error) > window:
            ...
            rolling_std = np.array([np.std(error[max(0, i-window//2):min(len(error), i+window//2)])
                                   for i in range(len(error))])

            # Plot
            ax.plot(timestamps, rolling_mean, color='blue', linewidth=2, label='Rolling Mean')
            ax.fill_between(timestamps, rolling_mean - rolling_std, rolling_mean + rolling_std,
                           color='blue', alpha=0.2, label='Rolling Std')
    ...
    plt.suptitle(f'Rolling Statistics (window={window}) - Error Stability Analysis',
