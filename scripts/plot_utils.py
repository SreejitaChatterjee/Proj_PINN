...
class PlotGenerator:
    ...
    def plot_time_series(self, data, states, title_prefix='', n_cols=4):
        ...
        for i, state in enumerate(states):
            if state in data.columns:
                ...
                axes[i].set(xlabel='Time (s)', ylabel=state.title(), title=f'{title_prefix}{state}')
    ...
    def plot_state_comparison(self, df_true, df_pred, states, save_prefix='comparison'):
        for i, state in enumerate(states, 1):
            ...
            ax.plot(df_pred['timestamp'], df_pred[state], 'r--', label='Predicted', alpha=0.7)
            ax.set(xlabel='Time (s)', ylabel=state.title(), title=f'{state.title()} Prediction')
            ax.legend(); ax.grid(alpha=0.3)
            plt.savefig(self.detailed_dir / f'{i:02d}_{state}_time_analysis.png', dpi=300, bbox_inches='tight')
            plt.close()

    def plot_parameter_convergence(self, param_history, true_values, save_name='param_convergence.png'):
    ...
    def plot_summary(self, results, save_name='summary.png'):
        ...
        for i, state in enumerate(states):
            if state in results['true'].columns:
                axes[i].plot(results['true']['timestamp'], results['true'][state], 'b-', label='True', alpha=0.6)
                axes[i].plot(results['pred']['timestamp'], results['pred'][state], 'r--', label='Pred', alpha=0.6)
                axes[i].set(xlabel='Time (s)', ylabel=state.title(), title=state.title())
                axes[i].legend(loc='best'); axes[i].grid(alpha=0.3)

        plt.suptitle('Quadrotor State Predictions vs Ground Truth', fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(self.output_dir / save_name, dpi=300, bbox_inches='tight')
        plt.close()
