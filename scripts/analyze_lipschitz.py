...
from pinn_architectures import BaselinePINN, ModularPINN, FourierPINN, CurriculumPINN
...
def analyze_jacobian_structure(model, x):
    ...
    return {
        ...
        'diagonal_coupling': (torch.norm(J_trans_trans, 'fro') + torch.norm(J_rot_rot, 'fro')).item()
    }
...
def main():
    ...
    for name, model in architectures.items():
        ...
        print(f"  Cross-coupling (Trans<->Rot):   {struct['trans_rot_spectral']:.4f} / {struct['rot_trans_spectral']:.4f}")
