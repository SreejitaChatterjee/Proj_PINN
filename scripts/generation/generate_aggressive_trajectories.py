...
def generate_aggressive_inertia_trajectories():
    ...
    for traj_id, traj_config in enumerate(aggressive_trajectories):
        ...
        print(
            f"  Roll:  period={traj_config['phi'][0]:.1f}s, range=[{traj_config['phi'][1]*180/np.pi:.1f}°, {traj_config['phi'][2]*180/np.pi:.1f}°]"
        )
        print(
            f"  Pitch: period={traj_config['theta'][0]:.1f}s, range=[{traj_config['theta'][1]*180/np.pi:.1f}°, {traj_config['theta'][2]*180/np.pi:.1f}°]"
        )
        print(
            f"  Yaw:   period={traj_config['psi'][0]:.1f}s, range=[{traj_config['psi'][1]*180/np.pi:.1f}°, {traj_config['psi'][2]*180/np.pi:.1f}°]"
        )
        print(
            f"  Alt:   period={traj_config['z'][0]:.1f}s, range=[{traj_config['z'][1]:.1f}m, {traj_config['z'][2]:.1f}m]"
        )
        ...
        # Add trajectory ID
        traj_data["trajectory_id"] = (
            traj_id + 100
        )  # Start from 100 to distinguish from standard trajectories
