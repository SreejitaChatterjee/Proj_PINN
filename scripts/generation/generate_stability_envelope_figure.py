...
def plot_stability_envelope(output_dir):
    ...
    for name, err in errors.items():
        ax1.semilogy(
            ...
            linewidth=2.0 if name == "Modular" else 1.5,
            label=name,
        )
    ...
    for name in ["Baseline", "Fourier", "Modular"]:
        ...
        print(
            f"{name:12s}: eps=0.1m->{env[0.1]:3d}, eps=0.5m->{env[0.5]:3d}, eps=1.0m->{env[1.0]:3d}"
        )
...
def plot_physics_data_conflict_bias(output_dir):
    ...
    plt.suptitle(
        "Physics-Data Conflict Bias: More Excitation → Worse Parameters",
        fontsize=10,
        fontweight="bold",
        y=1.02,
    )
