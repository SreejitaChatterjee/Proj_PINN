...
for mag in [0.3, 0.5, 0.7, 1.0, 1.3, 1.5, 2.0, 2.5, 3.0]:
    for attack_type in ["drift", "bias", "noise", "jump"]:
        ...
        if attack_type == "drift":
            attack = base + np.linspace(0, 5 * mag, 500).reshape(-1, 1) * np.array(
                [1, 1, 0.5, 0, 0, 0, 0, 0, 0, 0, 0, 0]
            )
        elif attack_type == "bias":
            attack = base + np.array(
                [0, 0, 0, 0.05 * mag, 0.05 * mag, 0, 0.025 * mag, 0.025 * mag, 0, 0, 0, 0]
            )
...
all_results["Data Augmentation"] = {
    "recall": np.mean(list(results_aug.values())),
    "fpr": fpr_aug,
    "per_attack": results_aug,
}
...
all_results["Physics Features"] = {
    "recall": np.mean(list(results_phys.values())),
    "fpr": fpr_phys,
    "per_attack": results_phys,
}
...
all_results["Anomaly (IsoForest)"] = {
    "recall": np.mean(list(results_anom.values())),
    "fpr": fpr_anom,
    "per_attack": results_anom,
}
...
all_results["Threshold (3-sigma)"] = {
    "recall": np.mean(list(results_thresh.values())),
    "fpr": fpr_thresh,
    "per_attack": results_thresh,
}
...
all_results["Normalized Features"] = {
    "recall": np.mean(list(results_norm.values())),
    "fpr": fpr_norm,
    "per_attack": results_norm,
}
...
all_results["GradientBoost+Physics"] = {
    "recall": np.mean(list(results_gb.values())),
    "fpr": fpr_gb,
    "per_attack": results_gb,
}
...
all_results["Ensemble (Majority)"] = {
    "recall": np.mean(list(results_ensemble.values())),
    "fpr": fpr_ensemble,
    "per_attack": results_ensemble,
}
...
# Best method
best_name = max(
    all_results.keys(), key=lambda x: all_results[x]["recall"] - all_results[x]["fpr"] * 0.5
)
